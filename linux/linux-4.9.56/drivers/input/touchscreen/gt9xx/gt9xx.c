 ...
 *                  By Meta, 2014/01/14
 *       V2.3:
 *          1.add 1024x600 xinlilai touch screen
 *          @2019.10.30
 */
...
// #define _CTP_PM_ENABLED_ // BUG, the resume is not ok, need to do more @2019.10.31
...
static int ctp_detect(struct i2c_client *client, struct i2c_board_info *info)
{
    ...
    int  ret = -1;
    GTP_INFO("the adapter number is %d\n", adapter->nr);
    ...
    if(twi_id == adapter->nr){
            ...
            GTP_INFO("detect ret %d\n",ret);
            if(!ret){
                GTP_INFO("%s:I2C connection might be something wrong \n", __func__);
                return -ENODEV;
            }else{                  
...
s32 gtp_send_cfg(struct i2c_client *client)
{
    ...
    for (retry = 0; retry < 5; retry++)
    {
        ...
        GTP_INFO("@ %s %d , ret = %d",__FUNCTION__,__LINE__,ret);
        if (ret > 0) // 1: success
        {
            break;
        }
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    #define MAX_TOUCH_SCREEN_SIZE (9)
#if GTP_DRIVER_SEND_CFG
    s32 i = 0;
    u8 check_sum = 0;
    u8 opr_buf[16] = {0};
...
#endif
    {
        ...
        if (SUCCESS == ret) 
        {
            ...
            {
                ts->fw_error = 1;
                GTP_INFO("Firmware error, no config sent!\n");
                return -1;
            }
        }
    }
 ...
 //       ret = gtp_i2c_read_dbl_check(ts->client, GTP_REG_SENSOR_ID, &sensor_id, 1);
    dprintk(DEBUG_INIT,"CTP name : %s\n",config_info.name);
        if (!strcmp(config_info.name,"gt9271_mb976a9")){
            ...
            sensor_id = 5;
            dprintk(DEBUG_INIT,"gt9xx:sensor_id = %d\n",sensor_id);
        } else if (!strcmp(config_info.name,"xinlilai_lcd7")){
            sensor_id = 6;
            dprintk(DEBUG_INIT,"gt9xx:sensor_id = %d\n",sensor_id);
        } else if (!strcmp(config_info.name,"ZB90PS0011_V3")){
            sensor_id = 7;
            dprintk(DEBUG_INIT,"gt9xx:sensor_id = %d\n",sensor_id);
        } else if (!strcmp(config_info.name,"gt9271_hzy")){
            sensor_id = 8;
            dprintk(DEBUG_INIT,"gt9xx:sensor_id = %d\n",sensor_id);
         } else {           
            sensor_id = 9; // "ZB90PS0011_V3 as default"
            dprintk(DEBUG_INIT,"gt9xx:sensor_id = %d\n",sensor_id);
        }
        ret = SUCCESS;
        if (SUCCESS == ret)
        {
            if (sensor_id >= MAX_TOUCH_SCREEN_SIZE)
            {
                GTP_INFO("Invalid sensor_id(0x%02X), No Config Sent!\n", sensor_id);
                ts->pnl_init_error = 1;
                return -1;
            }
        }
        ...
        {
            GTP_INFO("Failed to get sensor_id, No config sent!\n");
            ts->pnl_init_error = 1;
            return -1;
        }
    ...
    {
        GTP_INFO("Config Group%d is INVALID CONFIG GROUP(Len: %d)! NO Config Sent! You need to check you header file CFG_GROUP section!\n", sensor_id+1, ts->gtp_cfg_len);
        ts->pnl_init_error = 1;
        return -1;
    }
...
#endif
    {
        ...
        {
            GTP_INFO("Failed to get ic config version!No config sent!\n");
            return -1;
        }
    }
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    {
        GTP_INFO("I2C check functionality failed.\n");
        return -ENODEV;
    }
    ts = kzalloc(sizeof(*ts), GFP_KERNEL);
    if (ts == NULL)
    {
        GTP_INFO("Alloc GFP_KERNEL memory failed.\n");
        return -ENOMEM;
    }
    ...
    if (ret < 0)
    {
        GTP_INFO("I2C communication ERROR!");
    }
    ...
    if (ret < 0)
    {
        GTP_INFO("GTP init panel failed.");
    }
...
//#if (!GTP_GESTURE_WAKEUP)
    if(!gtp_gesture_wakeup){
        GTP_INFO("ctp power off in suspend!!!\n");
...
if(!gtp_gesture_wakeup){
    __gpio_set_value(config_info.wakeup_gpio.gpio, 1);
    GTP_INFO("ctp power on in late resume!!!\n");
...
#endif
    {
        GTP_DEBUG("gtp send cfg in resume");
...
{
    s32 ret = -1;
    dprintk(DEBUG_INIT,"[M64]GTP driver init\n");
    if (input_sensor_startup(&(config_info.input_type))) {
        GTP_INFO("%s: ctp_fetch_sysconfig_para err.\n", __func__);
        return 0;
    } else {
        ret = input_sensor_init(&(config_info.input_type));
        if (0 != ret) {
            GTP_INFO("%s:ctp_ops.init_platform_resource err. \n", __func__);    
        }
    }
    
    if(config_info.ctp_used == 0){
        GTP_INFO("*** ctp_used set to 0 !\n");
        GTP_INFO("*** if use ctp,please put the sys_config.fex ctp_used set to 1. \n");
        return 0;
    }
