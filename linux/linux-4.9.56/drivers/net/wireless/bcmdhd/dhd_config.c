...
uint config_msg_level = 0x03; //CONFIG_ERROR_LEVEL @2019.11.1
...
dhd_conf_get_mac(dhd_pub_t *dhd, bcmsdh_info_t *sdh, uint8 *mac)
{
	...
	cis = MALLOC(dhd->osh, SBSDIO_CIS_SIZE_LIMIT);
	if (!cis) {
	...
	err = bcmsdh_cis_read(sdh, 0, cis, SBSDIO_CIS_SIZE_LIMIT);
	if (err) {
	...
	int def, bool down)
{
	...
	if (val >= def) {
		...
		if (cmd == WLC_SET_VAR) {
			...
			ret = dhd_wl_ioctl_cmd(dhd, cmd, &val, sizeof(val), TRUE, 0);
			if (ret < 0)
				CONFIG_ERROR(("%s: %s setting failed %d\n", __FUNCTION__, name, ret));
		}
	}
	...
	int len, bool down)
{
	...
	if (cmd == WLC_SET_VAR) {
		...
		ret = dhd_wl_ioctl_cmd(dhd, cmd, iovbuf, sizeof(iovbuf), TRUE, 0);
		if (ret < 0)
			CONFIG_ERROR(("%s: %s setting failed %d\n", __FUNCTION__, name, ret));
	} else {
		ret = dhd_wl_ioctl_cmd(dhd, cmd, buf, len, TRUE, 0);
		if (ret < 0)
			CONFIG_ERROR(("%s: %s setting failed %d\n", __FUNCTION__, name, ret));
	}
...
dhd_conf_set_country(dhd_pub_t *dhd)
{
	...
	printf("%s %s: set country %s, revision %d\n",__FILE__, __FUNCTION__,dhd->conf->cspec.ccode, dhd->conf->cspec.rev);
	printf("@%d, %s ret = %d \n", __LINE__, __FUNCTION__, bcmerror);
...
dhd_conf_get_country(dhd_pub_t *dhd, wl_country_t *cspec)
{
	...
	bcmerror = dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, cspec, sizeof(wl_country_t), FALSE, 0);
	if (bcmerror < 0)
...
dhd_conf_fix_country(dhd_pub_t *dhd)
{
	...
	bcmerror = dhd_wl_ioctl_cmd(dhd, WLC_GET_VALID_CHANNELS, valid_chan_list, sizeof(valid_chan_list), FALSE, 0);
	if (bcmerror < 0) {
	...
	if (bcmerror || ((band == WLC_BAND_AUTO || band == WLC_BAND_2G) &&
		...
		bcmerror = dhd_conf_set_country(dhd);
		if (bcmerror < 0) {
...
dhd_conf_get_wme(dhd_pub_t *dhd, edcf_acparam_t *acp)
{
	...
	bcmerror = dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, iovbuf, sizeof(iovbuf), FALSE, 0);
	if (bcmerror < 0) {
...
dhd_conf_read_fw_by_mac(dhd_pub_t *dhd, char *bufp, uint len)
{
	...
	if (len_val) {
		...
		mac_list = kmalloc(sizeof(wl_mac_list_t)*conf->fw_by_mac.count, GFP_KERNEL);
		if (!mac_list) {
		...
		for (i = 0; i < conf->fw_by_mac.count; i++) {
			...
			mac_range = kmalloc(sizeof(wl_mac_range_t)*mac_list[i].count, GFP_KERNEL);
			if (!mac_range) {
				mac_list[i].count = 0;
				CONFIG_ERROR(("%s: kmalloc failed\n", __FUNCTION__));
				break;
...
dhd_conf_read_nv_by_mac(dhd_pub_t *dhd, char *bufp, uint len)
{
	...
	if (len_val) {
		...
		mac_list = kmalloc(sizeof(wl_mac_list_t)*conf->nv_by_mac.count, GFP_KERNEL);
		if (!mac_list) {
...
dhd_conf_read_nv_by_chip(dhd_pub_t *dhd, char *bufp, uint len)
{
	...
	if (len_val) {
		...
		chip_nv_path = kmalloc(sizeof(wl_mac_list_t)*conf->nv_by_chip.count, GFP_KERNEL);
		if (!chip_nv_path) {
...
dhd_conf_attach(dhd_pub_t *dhd)
{
	...
	conf = MALLOC(dhd->osh, sizeof(dhd_conf_t));
	if (!conf) {
