...
int strnicmp(const char *s1, const char *s2, __kernel_size_t n)
{
	if (!n)
		return 0;
	while ((--n) && (*s1) && (*s1 == *s2)) {
...
)
{
	...
	if (on) {
		...
		printf("%s sunxi wlan set power up\n",__FUNCTION__);
#endif
		...
		printf("%s sunxi wlan set power down\n",__FUNCTION__);
#endif
	}
...
static int dhd_wlan_set_carddetect(bool present)
{
	...
	printf("%s sunxi wlan get bus index %d\n",__FUNCTION__,wlan_bus_index);
	if(wlan_bus_index < 0)
	...
	if (present) {
		...
		printf("%s sunxi wlan mmc rescan card\n",__FUNCTION__);
#endif
#elif defined(BCMPCIE)
...
int dhd_wlan_init_gpio(void)
{
	...
	*/
	...
	printf("%s sunxi wlan get oob irq = %d\n",__FUNCTION__,host_oob_irq);
#endif
	...
	printf("%s sunxi wlan get oob irq flags = %d \n",__FUNCTION__, host_oob_irq_flags);
#endif
