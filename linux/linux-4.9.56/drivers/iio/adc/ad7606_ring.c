...
{
	struct ad7606_state *st = container_of(work_s, struct ad7606_state, poll_work);
	...
	if (gpio_is_valid(st->pdata->gpio_frstdata)) {
		...
		ret = st->bops->read_block(st->dev,st->chip_info->num_channels - 1, buf + 2);
		if (ret){
			goto done;
		}
	} else
#endif
	 {
		ret = st->bops->read_block(st->dev,st->chip_info->num_channels, buf);
		if (ret){
			goto done;
		}
	}
