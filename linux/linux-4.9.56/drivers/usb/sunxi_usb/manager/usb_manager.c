...
static int usb_device_scan_thread(void *pArg)
{
	...
	msleep(3000);
	DMSG_INFO("usb_device_scan_thread @%d\n", __LINE__);
...
static int usb_host_scan_thread(void *pArg)
{
	/* delay for udc & hcd ready */
	msleep(3000);
	DMSG_INFO("usb_host_scan_thread @%d\n", __LINE__);
...
static int usb_hardware_scan_thread(void *pArg)
{
	...
	DMSG_INFO("usb_hardware_scan_thread @%d\n", __LINE__);
...
static int usb_id_irq_thread(void *parg)
{
	...
	while (thread_id_irq_run_flag) {
		...
		if (cfg->port.id.valid) {
			...
			ret = request_threaded_irq(id_irq_num, NULL, usb_id_irq, irq_flags, "usb_id", cfg);
...
static __s32 usb_script_parse(struct device_node *np, struct usb_cfg *cfg)
{
	...
	if (ret){
		...
		DMSG_INFO("get usb_port_type is %d\n", cfg->port.port_type);
	}
	...
	if (ret){
		...
		DMSG_INFO("get usb_detect_mode is %d\n", cfg->port.detect_mode);
	}
...
int usb_otg_id_status(void)
{
	...
	if (cfg->port.port_type == USB_PORT_TYPE_OTG) {
		if (cfg->port.id.valid){
			id_status = __gpio_get_value(cfg->port.id.gpio_set.gpio);
		}
	}
...
static int sunxi_otg_manager_probe(struct platform_device *pdev)
{
	...
	dev_err(&pdev->dev,"sunxi_otg_manager_probe \n");
	...
	dev_err(&pdev->dev,"sunxi_otg_manager_probe %d\n",__LINE__);
	...
	if (g_usb_cfg.port.port_type == USB_PORT_TYPE_DEVICE) {
		dev_err(&pdev->dev,"sunxi_otg_manager_probe device port type is device \n");
	...
	if (g_usb_cfg.port.port_type == USB_PORT_TYPE_HOST) {
		dev_err(&pdev->dev,"sunxi_otg_manager_probe device port type is host \n");
	...
	if (g_usb_cfg.port.port_type == USB_PORT_TYPE_OTG) {
		dev_err(&pdev->dev,"sunxi_otg_manager_probe device port type is otg \n");

		usb_hw_scan_init(&g_usb_cfg);

		if (g_usb_cfg.port.detect_mode == USB_DETECT_MODE_THREAD) {
			dev_err(&pdev->dev,"sunxi_otg_manager_probe detct mode is thread \n");
			...
			dev_err(&pdev->dev,"sunxi_otg_manager_probe detct mode is irq \n");

			thread_id_irq_run_flag = 1;
			id_irq_th = kthread_create(usb_id_irq_thread, &g_usb_cfg, "usb_id_irq");
			if (IS_ERR(id_irq_th)) {
