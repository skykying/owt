...
static void insmod_device_driver(struct usb_msg_center_info *center_info)
{
	...
	DMSG_INFO("[%s] sunxi_usb_device_enable\n",__func__);
...
#if IS_ENABLED(CONFIG_USB_SUNXI_UDC0)
	DMSG_INFO("[%s]sunxi_usb_device_disable\n",__func__);
	sunxi_usb_device_disable();
#endif

#if defined(CONFIG_AW_AXP)
	DMSG_INFO("axp_usbcur & axp_usbvol %s\n",__func__);
