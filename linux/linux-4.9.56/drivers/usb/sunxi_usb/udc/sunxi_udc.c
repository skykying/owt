...
static ssize_t ed_test(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t udc_queue_debug(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int debug = 0;
	int ret = 0;
...
static ssize_t udc_dma_debug(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int debug = 0;
	int ret = 0;
...
static ssize_t udc_read_debug(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int debug = 0;
	int ret = 0;
...
static ssize_t udc_msc_write_debug(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int debug = 0;
	int ret = 0;
...
static ssize_t udc_msc_read_debug(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	int debug = 0;
	int ret = 0;
...
static ssize_t show_enable_udc_flag(struct device *dev,struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", is_udc_enable);
}

static ssize_t set_enable_udc_flag(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	int enable = 0;
	int ret = 0;
	...
	is_udc_enable = enable;

	return count;
}

static DEVICE_ATTR(udc_enable, 0644,show_enable_udc_flag, set_enable_udc_flag);
		...
		struct sunxi_udc_ep *ep)
{
	...
	if (req->req.dma == DMA_ADDR_INVALID) {
		...
		if (dma_mapping_error(udc->controller, req->req.dma)){
...
static void sunxi_udc_handle_ep0_idle(struct sunxi_udc *dev,struct sunxi_udc_ep *ep,struct usb_ctrlrequest *crq,u32 ep0csr)
{
...
static irqreturn_t sunxi_udc_irq(int dummy, void *_dev)
{
	...
	if (usb_irq & USBC_INTUSB_RESET) {
		...
		DMSG_INFO_UDC("axp_usbvol\n");
		axp_usbvol(CHARGE_USB_20);

		DMSG_INFO_UDC("axp_usbcur\n");
...
static int sunxi_udc_ep_enable(struct usb_ep *_ep,const struct usb_endpoint_descriptor *desc)
{
...
static int sunxi_udc_dequeue(struct usb_ep *_ep, struct usb_request *_req)
{
	...
	DMSG_INFO_UDC("dequeue: ep(0x%p, %d), _req(0x%p, %d, %d)\n",
		ep, ep->num,_req, _req->length, _req->actual);
...
{
	u32 i = 0;
	DMSG_DBG_UDC("sunxi_udc_reinit called\n");
...
static int sunxi_udc_start(struct usb_gadget *g, struct usb_gadget_driver *driver)
{
	struct sunxi_udc *udc = the_controller;

	DMSG_PANIC("ERR:[%s] begin [%d]\n",__func__,__LINE__);
	...
	if (udc->driver) {
		DMSG_PANIC("ERR:[%s] udc->driver is not null [%d]\n",__func__,__LINE__);
		return -EBUSY;
	}
...
static struct sunxi_udc sunxi_udc = {
	...
	.ep[1] = {
		...
		.ep = {
			...
			.caps	= USB_EP_CAPS(USB_EP_CAPS_TYPE_BULK,USB_EP_CAPS_DIR_IN),
		},
		.dev		        = &sunxi_udc,
	...
	.ep[2] = {
		...
		.ep = {
			...
			.maxpacket	= SW_UDC_EP_FIFO_SIZE,
			.maxpacket_limit = SW_UDC_EP_FIFO_SIZE,
			.caps	= USB_EP_CAPS(USB_EP_CAPS_TYPE_BULK,USB_EP_CAPS_DIR_OUT),
		},
		.dev		        = &sunxi_udc,
	...
	.ep[5] = {
		...
		.ep = {
			...
			.caps	= USB_EP_CAPS(USB_EP_CAPS_TYPE_ISO,USB_EP_CAPS_DIR_ALL),
		},
		.dev		        = &sunxi_udc,
		.bEndpointAddress   = 3,
...
static void sunxi_vbus_det_work(struct work_struct *work)
{
	...
	if (udc->driver && udc->driver->disconnect) {
		...
		while (axp_usb_is_connected()) {
			msleep(1000);
			DMSG_INFO_UDC("[%s]wait the usb to connected !! %d\n",__func__,__LINE__);

			/* after resume */
			if (usb_connect){
				DMSG_INFO_UDC("[%s]usb connected !! %d\n",__func__,__LINE__);
				return;
			}
		}
#endif
...
int get_dp_dm_status_normal(void)
{
	...
	DMSG_INFO_UDC("[%s] checks usb vbase !! %d\n",__func__,__LINE__);

	if (g_sunxi_udc_io.usb_vbase == NULL){
		return 0;
	}

	DMSG_INFO_UDC("[%s]enable dp dm !! %d\n",__func__,__LINE__);
...
int sunxi_usb_device_enable(void)
{
	...
	dev_err(&pdev->dev,"sunxi_usb_device_enable start \n");
	...
	dev_err(&pdev->dev,"sunxi_udc_bsp_init %d \n",__LINE__);
	...
	dev_err(&pdev->dev,"sunxi_udc_disable %d \n",__LINE__);
	sunxi_udc_disable(udc);

	dev_err(&pdev->dev,"platform_get_irq %d \n",__LINE__);
	...
	dev_err(&pdev->dev,"sunxi_udc_dma_probe %d \n",__LINE__);
	if (is_udc_support_dma()) {
	...
	dev_err(&pdev->dev,"INIT_WORK %d \n",__LINE__);
	INIT_WORK(&udc->vbus_det_work, sunxi_vbus_det_work);

	dev_err(&pdev->dev,"request_irq %d \n",__LINE__);
	retval = request_irq(udc->irq_no, sunxi_udc_irq, 0, gadget_name, udc);
	if (retval != 0) {
	...
	if (udc->driver && is_udc_enable) {
		dev_err(&pdev->dev,"sunxi_udc_enable %d \n",__LINE__);
		sunxi_udc_enable(udc);

		dev_err(&pdev->dev,"cfg_udc_command %d \n",__LINE__);
		cfg_udc_command(SW_UDC_P_ENABLE);
	}

	DMSG_INFO_UDC("sunxi_usb_device_enable end\n");
	dev_err(&pdev->dev,"sunxi_usb_device_enable end %d \n",__LINE__);

	return 0;
err:
	dev_err(&pdev->dev,"sunxi_usb_device_enable error %d\n",__LINE__);
	if (is_udc_support_dma()){
...
__acquires(sunxi_udc.lock)
{
	...
	dev_err(&pdev->dev,"sunxi_usb_device_disable start %d\n",__LINE__);
	...
	dev_err(&pdev->dev,"sunxi_usb_device_disable end %d\n",__LINE__);
...
static int sunxi_udc_probe_otg(struct platform_device *pdev)
{
	...
	dev_err(&pdev->dev,"sunxi_udc_probe_otg \n");
	...
	dev_err(&pdev->dev,"sunxi_udc_is_enable \n");
	...
	dev_err(&pdev->dev,"sunxi_get_udc_resource \n");
	retval = sunxi_get_udc_resource(pdev, &g_sunxi_udc_io);
	if (retval != 0) {
		DMSG_PANIC("ERR: sunxi_get_udc_resource, is fail\n");
		return -ENODEV;
	...
	dev_err(&pdev->dev,"sunxi_udc_io_init \n");
	sunxi_udc_io_init(usbd_port_no, &g_sunxi_udc_io);

	dev_err(&pdev->dev,"sunxi_udc_reinit \n");
	sunxi_udc_reinit(udc);

	the_controller = udc;

	dev_err(&pdev->dev,"platform_set_drvdata \n");
	platform_set_drvdata(pdev, udc);

	dev_err(&pdev->dev,"usb_add_gadget_udc \n");
	...
	dev_err(&pdev->dev,"device_create_file \n");
	device_create_file(&pdev->dev, &dev_attr_udc_enable);
...
static int sunxi_udc_remove_otg(struct platform_device *pdev)
{
	...
	dev_err(&pdev->dev,"sunxi_udc_remove_otg \n");
	udc = platform_get_drvdata(pdev);
	if (udc->driver) {
	...
	dev_err(&pdev->dev,"device_remove_file \n");
	device_remove_file(&pdev->dev, &dev_attr_udc_enable);
	...
	dev_err(&pdev->dev,"usb_del_gadget_udc \n");
	usb_del_gadget_udc(&udc->gadget);

	dev_err(&pdev->dev,"sunxi_udc_io_exit \n");
	sunxi_udc_io_exit(&g_sunxi_udc_io);
	memset(&g_sunxi_udc_io, 0, sizeof(sunxi_udc_io_t));
